...
2. **Low RAM** — Pi 3 has 1GB RAM. Build requires a 2GB swapfile and:
   - `-j1` for the pjproject stage
   - `-j2` for the main Asterisk build

## Build flags

```bash
./configure --with-pjproject-bundled --with-jansson-bundled --with-libjwt-bundled --disable-xmldoc CFLAGS="-O1 -pipe -fno-strict-aliasing -g0"
```
...
```bash
sudo apt-get install -y   libbsd0 libc6 libcap2 libcrypt1 libedit2   libgcc-s1 liblzma5 libmd0 libsqlite3-0   libssl3t64 libstdc++6 libtinfo6 liburiparser1   libuuid1 libxml2 libxslt1.1 libzstd1 zlib1g   libsrtp2-1 libspandsp2 libjansson4 libgsm1   libspeex1 libspeexdsp1 libogg0 libvorbis0a
```
